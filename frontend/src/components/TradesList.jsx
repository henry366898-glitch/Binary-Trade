...
function Countdown({ expiresAt }) {
  ...
  if (ms <= 0) return <span className="tag open">settling…</span>;
          ...
          <tbody>
            ...
            {trades.map((t) => (
              <tr key={t.id} className={t.status === 'open' ? 'row-open' : ''}>
                ...
                <td>
                  {t.status === 'open'
                    ? <Countdown expiresAt={t.expires_at + 'Z'} />
                    : fmtTime(t.expires_at)}
                </td>
                <td><span className={`tag ${t.status}`}>{t.status}</span></td>
                <td style={{ color: t.status === 'won' ? 'var(--up)' : t.status === 'lost' ? 'var(--down)' : 'inherit' }}>
                  {t.status === 'open'
                    ? '—'
